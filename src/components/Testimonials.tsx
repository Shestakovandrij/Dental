      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          <div className="anim-fade-up bg-gradient-to-br from-primary to-primary-dark rounded-3xl p-8 lg:p-10 flex flex-col justify-between relative overflow-hidden lg:row-span-2">
            <div className="absolute top-4 right-6 text-white/10 text-[100px] leading-none font-serif pointer-events-none">&ldquo;</div>
            <div>
              <Stars count={testimonials[0].rating} />
              <p className="text-white text-xl lg:text-2xl leading-relaxed mt-6 mb-8 relative z-10 font-light">&ldquo;{testimonials[0].text}&rdquo;</p>
            </div>
            <div className="flex items-center gap-4 relative z-10">
              <div className="w-12 h-12 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center text-white font-bold">{testimonials[0].initials}</div>
              <div>
          ...
          {visible.slice(1).map((item, i) => (
            <div key={i} className="anim-fade-up bg-gray-900 rounded-3xl p-8 group hover:bg-gray-800 transition-colors duration-300">
              <Stars count={item.rating} />
              <p className="text-white/80 text-lg leading-relaxed mt-4 mb-6 font-light">&ldquo;{item.text}&rdquo;</p>
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 bg-accent/20 text-accent rounded-full flex items-center justify-center text-sm font-bold">{item.initials}</div>
                <div>
        ...
        {testimonials.length > VISIBLE_COUNT && (
          <div className="text-center mt-10 anim-fade-up">
            <button onClick={() => setShowAll(!showAll)} className="inline-flex items-center gap-2 text-white/60 hover:text-white font-medium transition-colors border border-white/20 hover:border-white/40 rounded-full px-8 py-3">
              {showAll ? "Згорнути" : `Показати ще ${testimonials.length - VISIBLE_COUNT}`}
              <svg className={`w-4 h-4 transition-transform ${showAll ? "rotate-180" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" /></svg>
            </button>
          </div>
        )}
      </div>
